MODULE step
   ...
   !! $Id: step.F90 10126 2018-09-13 14:59:46Z jchanut $
...
!!gm kcall can be removed, I guess
      ...
      INTEGER :: ln_const_den   ! DB: parameter for controlling constant density
      !! ---------------------------------------------------------------------
#if defined key_agrif
...
# endif
#endif
      ln_const_den = 1 !DB: For now just hardcode this
      !
      ...
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      !  THERMODYNAMICS
      IF( ln_const_den .EQ. 0) THEN !DB: Remove calls to e.o.s and buoyancy frequency
      ...
      IF(ln_const_den .EQ. 0) CALL eos    ( tsn, rhd, rhop, gdept_n(:,:,:) )  ! now in situ density for hpg computation
      !DB: above, remove call to e.o.s                      
...
!!jc: fs simplification
      ...
      IF( ln_dynspg_ts ) THEN                         ! vertical scale factors and vertical velocity need to be updated
         ...
         IF(.NOT.ln_linssh) CALL dom_vvl_sf_nxt( kstp, kcall=2 )  ! after vertical scale factors (update depth average component)
                            CALL wzv        ( kstp, kcall=2 )     ! now cross-level velocity 
      ENDIF
      ...
      ! Active tracers                              
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      IF(ln_const_den .EQ. 0) THEN !DB: Remove calls to tracer processes                   
...
!!jc2: dynnxt must be the latest call. e3t_b are indeed updated in that routine
      !DB: Remove tracer call (below)
      IF(ln_const_den .EQ. 0) CALL tra_nxt       ( kstp )  ! finalize (bcs) tracer fields at next time step and swap
